...
#sprawdzamy pamięć ram
	...
	typram=$(sudo dmidecode -t 17 |grep -i Type | head -2| cut -c-11 | grep -i type| awk '{print $2}')
	czestotliwoscram=$(sudo dmidecode -t 17 | grep -i speed |head -n1| cut -d: -f2)
...
printf "%-1s %-15s %-1s %-40s %-1s  %s\n" "" "Dyski HDD" "-" "Calkowita > $hddfull | Zajeta > $hddzajete | Dostepna > $hdddostepne" "" 
printf "%-1s %-15s %-1s %-40s %-1s  %s\n" "" "Inne napędy" "-" "usb > $usb" "" 
